...
AUTO_START_HOUR = 19      # Hora en formato 24h (17 = 5 PM)
AUTO_START_MINUTE = 41    # Minuto (0-59)
...
async def iniciar_tiempo(interaction: discord.Interaction, usuario: discord.Member):
    ...
    if current_hour > AUTO_START_HOUR or (current_hour == AUTO_START_HOUR and current_minute >= AUTO_START_MINUTE):
        ...
        if success:
            ...
            await interaction.response.send_message(
                f"⏰ **INICIO INMEDIATO** - Son las {colombia_now.strftime('%H:%M')} (Colombia)\n"
...
async def add_attendance_for_milestone(member: discord.Member, hours_completed: int):
    """Agregar asistencia cuando alguien completa milestone - solo 1 asistencia por milestone individual"""
    try:
        print(f"🔍 Verificando asistencia para milestone de {member.display_name} ({hours_completed} horas)")

        # Obtener información de quién inició el tiempo desde el registro
        ...
        # Verificar si el iniciador transfirió asistencias hoy (NO debe recibir asistencias)
        if not time_tracker.can_receive_daily_attendance(admin_id):
            print(f"🚫 {admin_name} no puede recibir asistencias porque transfirió hoy - NO se agregará asistencia")
            return
        ...
        if admin_member:
            if has_attendance_role(admin_member):
                ...
                if success:
                    ...
                    print(f"✅ Asistencia agregada: {admin_member.display_name} (+1) por completar hora {hours_completed} de {member.display_name if member else initiator_info.get('admin_name', 'Usuario externo')}")
                else:
